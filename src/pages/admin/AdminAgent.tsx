  ...
  const send = async (text?: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-agent", {
        body: { message: content, history, session_id: SESSION_ID },
      });
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Gestiona el catálogo conversando. El agente busca, crea y edita productos por ti. Cada acción queda registrada.
        </p>
      </div>
